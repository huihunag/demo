...
public class ExcelUtil<T> {
    ...
    public Cell addCell(Excel attr, Row row, T vo, Field field, int column) {
        ...
        try {
            ...
            if (attr.isExport()) {
                ...
                int second = attr.secondToMin().value();
                if (StringUtils.isNotEmpty(dateFormat) && StringUtils.isNotNull(value)) {
                    cell.setCellValue(DateUtils.parseDateToStr(dateFormat, (Date) value));
                } else if (StringUtils.isNotEmpty(readConverterExp) && StringUtils.isNotNull(value)) {
                    cell.setCellValue(convertByExp(String.valueOf(value), readConverterExp));
                } else if (second != 0) {
                    if (second == Excel.TimeType.MINUTE.value()) {
                        cell.setCellValue((int) value / 60);
                    } else if (second == Excel.TimeType.HOUR.value()) {
                        cell.setCellValue((int) value / 60 / 60);
                    } else if (second == Excel.TimeType.DAY.value()) {
                        cell.setCellValue((int) value / 60 / 60 / 24);
                    }
                } else {
                    // 设置列类型
    ...
    public String getAbsoluteFile(String filename) {
        String downloadPath = System.getProperty("user.dir") + download + "/" + filename;
